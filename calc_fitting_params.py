...
def single_fit(m_structure_list, fit_int, Monomer_rules,Cluster_rules, J_rules):
    predictors = []  # used to be a
    data = []  # used to be b
    for i in range(len(m_structure_list)):
        mat = m_structure_list[i]
        row = mat.Monomer_sums + mat.Cluster_sums + mat.J_sums
    ...
    col = ['rss', 'intercept'] + ['%s' % Monomer_rules[i].name.rstrip() for i in range(len(Monomer_rules))] + ['%s' % Cluster_rules[i].name.rstrip() for i in range(len(Cluster_rules))] + [
        '%s' % J_rules[i].name.rstrip() for i in range(len(J_rules))]
    ind = ['alpha_%.2g' % alpha_list[i] for i in range(len(alpha_list))]
    coef_matrix_ridge = pd.DataFrame(index=ind, columns=col)

    plt.figure
    models_to_plot = {alpha_list[0]: 231, alpha_list[10]: 232, alpha_list[20]: 233, alpha_list[30]: 234,
                      alpha_list[40]: 235, alpha_list[50]: 236}
    ...
    ridge_fit = linear_model.RidgeCV(alphas=alpha_list, fit_intercept=fit_int, normalize=True)
    ridge_fit.fit(predictors, data)
    #ridge_fit = linear_model.Ridge(alpha=305.678730, fit_intercept=fit_int, normalize=True)
    #ridge_fit.fit(predictors, data)
    ...
    if fit_int == False: # swap commented lines if want intercept
        intercept = 0
    return Js, intercept

def first_fit(m_structure_list, fit_int ,Monomer_Rules):
    predictors = []  # used to be a
    data = []  # used to be b
    for i in range(len(m_structure_list)):
        mat = m_structure_list[i]
        row = mat.Monomer_sums
        for j in range(len(row)):
            row[j] *= mat.weight
        predictors.append(row)
        data.append(mat.enrg * mat.weight)
...
def second_fit(m_structure_list, fit_int ,Cluster_rules, J_rules):
    predictors = []  # used to be a
    data = []  # used to be b
    for i in range(len(m_structure_list)):
        mat = m_structure_list[i]
    ...
    # initialize dataframe for storing coefficients
    col = ['rss', 'intercept'] + ['%s' % Cluster_rules[i].name.rstrip() for i in range(len(Cluster_rules))] + [
        '%s' % J_rules[i].name.rstrip() for i in range(len(J_rules))]
    ind = ['alpha_%.2g' % alpha_list[i] for i in range(len(alpha_list))]
    coef_matrix_ridge = pd.DataFrame(index=ind, columns=col)
...
def beg_fit(m_structure_list, fit_int):
    predictors = []  # used to be a
    data = []  # used to be b
    for i in range(len(m_structure_list)):
        mat = m_structure_list[i]
        row = mat.Cluster_sums
        for j in range(len(row)):
            row[j] *= mat.weight
        predictors.append(row)
        data.append(mat.enrg * mat.weight)
...
def spin_fit(m_structure_list, fit_int):
    predictors = []  # used to be a
    data = []  # used to be b
    for i in range(len(m_structure_list)):
        mat = m_structure_list[i]
...
def write_fitting_parameters(structures, monomer_list, clusters_list, j_list, Js, intercept, limit):
    label = []
    for i in range(len(monomer_list)):
        label.append(monomer_list[i].name)
    ...
    for i in range(len(Js)):
        ...
        file.write(line)
    file.write("\n")
    file.write("Name".ljust(20) + "Actual Energy".ljust(20) + "Predicted Energy".ljust(20) + "Magnetic Order".ljust(
        20) + "Phase".ljust(20) + "Weight".ljust(20) + "\n")
    for i in range(len(structures)):  # seems like energy evaluation should just be a function we can call
        mat = structures[i]
        #        if mat.phase_name != "pmmmm":
        file.write(str(mat.name).ljust(20) + str(mat.original_enrg).ljust(20))
        new_enrg = intercept
        for j in range(len(monomer_list)):
            new_enrg += Js[j] * structures[i].Monomer_sums[j]
        for k in range(len(clusters_list)):
            new_enrg += Js[1 + j + k] * structures[i].Cluster_sums[k]
        for l in range(len(j_list)):
            new_enrg += Js[2 + j + k + l] * structures[i].J_sums[l]
        line = str(new_enrg)
        line = line.replace("[", "")
        line = line.replace("]", "")
        file.write(
            line.ljust(20) + mat.mag_phase.ljust(20) + mat.phase_name.ljust(20) + str(mat.weight).ljust(20) + "\n")
    file.close()

def linearize(structures, type, rule_list, Js, intercept):
    if type == "MONOMER":
        for i in range(len(structures)):  # seems like energy evaluation should just be a function we can call
            new_enrg = structures[i].enrg - intercept
            for k in range(len(rule_list)):
                new_enrg -= Js[k] * structures[i].Monomer_sums[k]
            structures[i].enrg = new_enrg
    if type == "CLUSTER":
        for i in range(len(structures)):  # seems like energy evaluation should just be a function we can call
            new_enrg = structures[i].enrg - intercept
            for k in range(len(rule_list)):
    ...
    if type == "SPIN":
        for i in range(len(structures)):  # seems like energy evaluation should just be a function we can call
            new_enrg = structures[i].enrg - intercept
            for k in range(len(rule_list)):
                new_enrg -= Js[k] * structures[i].J_sums[k]
            structures[i].enrg = new_enrg
# plotting function not written very generally here, just does NiMnIn
def plot_data3(M_structures, monomer_list, clusters_list, j_list, Js, intercept, limit, hull):
    ...
    for i in range(len(M_structures)):
        if M_structures[i].phase_name != "pmmmm":
            ...
            if hull == True:
                enrg_list.append(M_structures[i].original_enrg)
            else:
                enrg_list.append(M_structures[i].original_enrg - (offset * comp + comp0_min))
    new_enrg_list = []
    for i in range(len(M_structures)):
        mat = M_structures[i]
        ...
        if mat.phase_name != "pmmmm":
            new_enrg = intercept
            for j in range(len(monomer_list)):
                new_enrg += Js[j] * M_structures[i].Monomer_sums[j]        # ugh - here evaluating it again, now above hull
            for k in range(len(clusters_list)):
                new_enrg += Js[1 + j + k] * M_structures[i].Cluster_sums[k]
            for l in range(len(j_list)):
                new_enrg += Js[2 + j + k + l] * M_structures[i].J_sums[l]
    ...
    for i in range(len(enrg_list)):
        if M_structures[i].phase_name != "pmmmm":
            ...
            if M_structures[i].phase_name != 'pm':
                ...
                plt.plot(x + x_itter, float(new_enrg_list[i])/16, lw=0, markersize=8, marker=".", color="r")
...
def plot_data4(M_structures, clusters_list, j_list, Js, intercept, limit, hull):
    plt.figure(5)  # do we really need to keep count of the number of figures?
    colors = {'mart': 'g', 'aus': 'b', 'pre-mart': 'r'}
    markers = {'none': 'x', 'FM': 'o', 'AFM': 's', 'spin disordered': 'D'}
    labels = ['NiMn', 'Ni4Mn3In1', 'Ni2MnIn']
    actual_labels = ['Ni$_8$Mn$_8$', 'Ni$_8$Mn$_6$In$_2$', 'Ni$_8$Mn$_4$In$_4$']
    ...
    for i in range(len(M_structures)):
        if M_structures[i].phase_name != "pmmmm":
            enrg_list.append(M_structures[i].enrg - intercept)
    new_enrg_list = []
    for i in range(len(M_structures)):
        mat = M_structures[i]
        new_enrg = 0
        if mat.phase_name != "pmmmm":
            for k in range(3,len(clusters_list)):
    ...
    # lgd = plt.legend(loc='center left', bbox_to_anchor=(1, 0.5), fontsize=12)
    plt.xticks([2, 5.5, 10], actual_labels, rotation='horizontal', fontsize=14)
    plt.savefig('FittedDataSummary2.pdf', bbox_inches='tight')
