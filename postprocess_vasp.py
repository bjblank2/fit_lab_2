...
def generate_m_structure(data_file, num_Monomer_rules, num_Cluster_rules, num_J_rules, aust_tol, spin_style, spin_tol, Monomer_rules, Cluster_rules, J_rules):
    m_struct_list = []
    ...
    for i in range(len(lines)):
        if '#' in lines[i]:
            ...
            m_struct = m_structure.MStructureObj(lines[i+1], species, num_Monomer_rules, num_Cluster_rules, num_J_rules, aust_tol)
            for j in range(m_struct.num_Atoms):
                atom_data = lines[i + j + 2]
                m_struct.set_atom_properties(j, atom_data, spin_style, spin_tol)
            #####################
            calculate_sums(m_struct, Monomer_rules, Cluster_rules, J_rules, spin_style, spin_tol)
...
def calculate_sums(m_structure_list, monomer_rule_list, cluster_rule_list, j_rule_list, spin_style, spin_tol):
    ...
    for j in range(m_structure_list.num_Atoms):
        for l in range(len(monomer_rule_list)):
            if m_structure_list.basis[j].species in monomer_rule_list[l].home_atom_list:        # Calc cluster sums monomers only
                if (monomer_rule_list[l].neighbor_order < 0.5):        # monomer contributions
                    if m_structure_list.phase_name == monomer_rule_list[l].phase or monomer_rule_list[l].phase == 'ALL':
                        m_structure_list.Monomer_sums[l] += 1
                    ...
                    if m_structure_list.basis[j].species in cluster_rule_list[l].home_atom_list:
                        if (m_structure_list.distances[j, k] == m_structure_list.mins[j, cluster_rule_list[l].neighbor_order - 1]):
                            if m_structure_list.check_plane(j, k) == cluster_rule_list[l].plane or m_structure_list.check_plane(j, k) == 'ALL' or cluster_rule_list[l].plane == 'ALL':
                                if m_structure_list.phase_name == cluster_rule_list[l].phase or cluster_rule_list[l].phase == 'ALL':
                                    if cluster_rule_list[l].neighbor_arrangement == 'COMB':
